...
```{r merge_plots, dependson=c("plot_scatter", "plot_boxplots"), fig.height=6.3, fig.width=4.2, warning=FALSE}
...
```{r stats, dependson="arrange_output"}
...
```{r uncertainty_plots2, fig.height=3, fig.width=6.4, warning=FALSE, dependson="scatters_plot"}

# PLOT SCATTERS WITH UNCERTAINTY ----------------------------------------------
...
b <- melt(final.output, measure.vars = discrepancy_methods) %>%
  ggplot(., aes(reorder(variable, -value), value)) +
  geom_boxplot() + 
  labs(x = "Discrepancy measures", y = "$r$") + 
...
legend <- get_legend(a + theme(legend.position = "top", 
                               legend.margin=margin(0, 0, 0, 0),
                               legend.box.margin=margin(-7,-7,-7,-7)))
bottom <- plot_grid(a, b, ncol = 1, labels = c("d", "e"))
...
```{r merged_plots, dependson=c("bratley_fun", "uncertainty_plots2"), fig.height=4.3, fig.width=6.4, warning=FALSE}

top.plots <- plot_grid(full.plot, bratley_plots, ncol = 1, rel_heights = c(0.75, 0.25))
...
plot_grid(top.plots, bottom.plots, ncol = 1, rel_heights = c(0.77, 0.23), 
          labels = c("", "g"))
```
\newpage
